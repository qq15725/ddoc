...
class Blueprint
{
    ...
    {
        $this->reader->addNamespace('Wxm\\DDoc\\Annotation');
        $this->reader->addNamespace('Wxm\\DDoc\\Annotation\\Method');

        AnnotationRegistry::registerLoader(function ($class) {
            $path = __DIR__ . '/' . str_replace(['Wxm\\DDoc\\', '\\'], ['', DIRECTORY_SEPARATOR], $class) . '.php';
    ...
    {
        $this->appendSection($contents, '请求参数');
        $contents .= $this->line();
        $contents .= '字段名称 | 字段类型 | 必填参数 | 说明';
        $contents .= $this->line();
        $contents .= '--- | --- | --- | ---';
        $contents .= $this->line();
        $parameters->each(function ($parameter) use (&$contents) {
            $descriptionExt = $parameter->example ? "例子：`{$parameter->example}`" : '';
            $descriptionExt .= $parameter->default ? "默认值：`{$parameter->default}`" : '';
            $description    = ($descriptionExt ? "（{$descriptionExt}）" : '') . $parameter->description;

            $contents .= sprintf(
                '%s | %s | %s | %s',
                $parameter->identifier,
                $parameter->members ? sprintf('enum[%s]', $parameter->type) : $parameter->type,
                $parameter->required ? 'Y' : 'N',
                $description
            );
            $contents .= $this->line();
...
//            }
     ...
     *
     * @param string                        $contents
     * @param \Wxm\DDoc\Annotation\Response $response
    ...
    {
        $this->appendSection($contents, sprintf('响应码：`%s`', $response->statusCode));
...
//            $contents .= '```';
//            $contents .= $this->line();
//            $contents .= "content-type：{$response->contentType}";
//            $contents .= $this->line();
//            $contents .= '```';
//            $contents .= $this->line();
//        }
     ...
     *
     * @param string                       $contents
     * @param \Wxm\DDoc\Annotation\Request $request
     * @param \Wxm\DDoc\RestResource       $resource
     *
     * @return void
     */
    ...
    {
        $this->appendSection($contents, '请求说明');
...
//            $contents .= '```';
//            $contents .= $this->line();
//            $contents .= "content-type：{$request->contentType}";
//            $contents .= $this->line();
//            $contents .= '```';
//            $contents .= $this->line();
//        }
